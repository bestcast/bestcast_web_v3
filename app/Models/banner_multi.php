...
class Banner extends Database implements RoleHasRelationsContract
{
    ...
    {
        return \App\Models\Post::whereIn('id', $this->page_id ?? [])->get();
    }
    ...
    {        
        ...
        /*$getpageid=app('request')->input('page_id');if(!empty($getpageid)){ //filter by page
            ...
            $data = $data->whereJsonContains('page_id', $getpageid);
        }*/
        $getpageid = app('request')->input('page_id');

        if (!empty($getpageid)) { // filter by page
            $data = $data->where(function ($query) use ($getpageid) {
                $query->where('page_id', $getpageid) // old INT values
                      ->orWhereJsonContains('page_id', (string)$getpageid); // JSON values
            });
        }
