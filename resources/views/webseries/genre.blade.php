@if(!empty($urlkey) && ($urlkey=='webseries'))
<?php
	$urlGuest='';
	$user=Auth::user();
			...
			<div class="container-fluid">
				<div class="row">
		         <div class="col-8 col-lg-8">
		         	<div class="label">{{ empty($post->title)?'Webseries':$post->title }}</div>
		         	@if($urlkey=='webseries')
			         			...
			         			@foreach ($chunks as $chunk)
		         					<ul>
				         				@foreach ($chunk as $item)
				         					<li><a class="{{ (!empty($genre->id) && ($genre->id==$item->id))?'active':'' }}" href="{{ url($urlGuest.'/webseries/genre/'.$item->id) }}">{{ $item->title }}</a></li>
									    @endforeach
		         					</ul>
			         			@endforeach
			         		</div>
		         ...
		         <div class="col-4 col-lg-4">
					...
					@if(!empty($languageslist) && count($languageslist))
			         	<div class="dvDropDw languagelist">
			         		...
			         		<div class="bxList cfix">
	         					<ul>
		         					<li><a class="{{ empty($language->id)?'active':'' }}" href="{{ url($urlGuest.'/webseries/language/0') }}">All</a></li>
			         				@foreach ($languageslist as $item)
			         					<li><a class="{{ (!empty($language->id) && ($language->id==$item->id))?'active':'' }}" href="{{ url($urlGuest.'/webseries/language/'.$item->id) }}">{{ $item->title }}</a></li>
								    @endforeach
	         					</ul>
			         		</div>
			         	</div>
