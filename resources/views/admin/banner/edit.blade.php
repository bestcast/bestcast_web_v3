...
@section('content')
{{ Form::model($model, ['route' => ['admin.banner.editsave', $model->id], 'method' => 'post']) }}

  <div class="row">
    <div class="col-md-8">
      <div class="container-fluid">
        @include('admin.common.message')
          <h2 class="pb-2 border-bottom">Edit Banner : {{ $model->title  }} </h2> <br>
          <div class="form-row">
          ...
          <div class="form-row form-select2 {{ empty($model->type)?'':'dnn' }}" id="movies_dropdown">
            ...
            @php ($selectedValue=empty($model->movies_id)?[]:[$model->movies_id=>$model->movies->title])
            {{ Form::select('movies_id', $selectedValue,old('movies_id',$model->movies_id), array('id'=>'movies_id','class' => 'form-select')); }}
          ...
          <div class="form-row form-select2 {{ !empty($model->type)?'':'dnn' }}"  id="shows_dropdown">
            ...
            @php ($selectedValue=empty($model->shows_id)?[]:[$model->shows_id=>$model->shows->title])
            {{ Form::select('shows_id', $selectedValue,old('shows_id',$model->shows_id), array('id'=>'shows_id','class' => 'form-select')); }}
            <!-- <select id="show_ids" name="show_ids[]" class="form-control" multiple></select> -->
            <script>
            jQuery(document).ready(function($) {
              $('#shows_id').select2({
          ...
          <div class="row pt-2 form-img-upload">
            ...
            <div class="col-3 themed-grid-col">
                {!! Field::mediaUpload('logo_id','Logo',$model) !!}
            </div>
          </div>
          <div class="row pt-2 form-img-upload">
            <div class="col-3 themed-grid-col">
            ...
            <div class="col-3 themed-grid-col">
                <p><b>Image</b> (1920X1080)<br><b>Thumbnail</b> (720X405)<br><b>Logo</b> (300X100)<br><b>Portrait Small</b> (400X600)<br><b>Portrait</b> (1000X1500)</p>
            </div>
          </div>
          ...
          <div class="card">
              ...
              <div class="card-body">
                  ...
                  <div class="form-row col-md-12">
                      <div class="form-row btnaction">
                          ...
                          <a href="{{ route('admin.banner.index') }}" class="btn btn-secondary backbtn">Back</a>
                      </div>
                  </div>
              </div>
              <div class="card-footer text-muted">
                ...
                <div class="deleteaction">
                  ...
                  @php ($delid=$model->id)
                  @php ($delurl=route('admin.banner.delete',$model->id))
